...
reference:"Surah At-Tawbah 9:129 (Recommended in Abu Dawood 5081)"
...
let currentPosition = currentDuaList.findIndex(
dua => dua === duas[index]
);


document.getElementById("readerCount").textContent =
(currentPosition + 1) + " / " + window.currentDuaList.length;

document.getElementById("readerFavBtn").innerHTML =
favorites.includes(index) ? "❤️" : "🤍";

document.getElementById("duaReader").classList.add("show");
...
function changeDuaTheme(theme){

document.body.className="dua-"+theme;

localStorage.setItem("duaTheme",theme);
...
function loadDuaTheme(){

let savedTheme=

localStorage.getItem("duaTheme");

if(savedTheme){

document.body.className="dua-"+savedTheme;
...
let currentPosition = currentDuaList.findIndex(
dua => dua === duas[currentDuaIndex]
);



if(currentPosition < currentDuaList.length - 1){


let nextDuaItem = currentDuaList[currentPosition + 1];


currentDuaIndex = duas.indexOf(nextDuaItem);


openReader(currentDuaIndex);
...
let previousDuaItem = currentDuaList[currentPosition - 1];


currentDuaIndex = duas.indexOf(previousDuaItem);
...
if(favorites.includes(currentDuaIndex)){
...
favorites.push(currentDuaIndex);
...
favorites.includes(currentDuaIndex)
...
let value = document
.getElementById("duaSearch")
.value
.toLowerCase();


let filtered = duas.filter(dua =>
...
result = duas.filter(dua=>{


return dua.category === category;
...
let element = document.getElementById("favoriteCount");
