...
async function initNativeNotifications() {
    ...
    try {
        ...
        /*
         * ==========================================
         * SINGLE CHANNEL: AZAN (azan.mp3 + vibration)
         * ==========================================
         * Ye channel azan.mp3 bajata hai aur vibration bhi karta hai.
         *
         * IMPORTANT: Azan.mp3 yahan hona chahiye:
         * android/app/src/main/res/raw/azan.mp3
         *
         * ✅ New channel ID (v5) — Android channel settings ek dafa
         * save hone ke baad change nahi hoti, is liye sound/
         * importance update karne ke liye channel ID badalna
         * zaroori hai.
         */
        try {
            ...
            await LocalNotifications.createChannel({
                ...
                description:
                    "Prayer time alarms with Azan sound",
...
// ======================================
// SCHEDULE PRAYER ALARM (single notification)
// ======================================
...
async function scheduleNativeAlarm(prayer, hour, minute, askPermission) {
    ...
    const azanId = PRAYER_ID_MAP[prayer];
    ...
    try {
        ...
        if (!granted) {
            ...
            if (askPermission) {
                ...
                showToast("⚠️ Exact alarm permission off — alarm late baj sakta hai");
            }
        ...
        /*
         * ==========================================
         * SINGLE AZAN NOTIFICATION (00 second)
         * ==========================================
         * Azan.mp3 bajegi + vibration hogi + heads-up
         * notification aayegi. Sound + vibration ke saath
         * Android khud screen wake karta hai (agar phone
         * settings me "Wake screen for notifications" ON ho).
         */
        const result =
        ...
        console.log(
            `✅ ${prayer} alarm scheduled daily at ` +
    ...
    if (alarms[prayer]) {
        ...
        if (time) {
            ...
            if (ok) {
                ...
                showToast(
                    `🔔 ${prayer} alarm on — roz ${time} par bajega`
                );
...
function checkAlarms() {
    ...
    if (nativeNotificationsReady && nativeExactOk) {
        return;
    }
...
function loadCachedData() {
    ...
    if (savedName) {
        ...
        const cleanName =
            normalizeLocationName(savedName);
    ...
    const cached =
        localStorage.getItem("cachedPrayerTimes");
    ...
    if (cached) {
        ...
        try {
            ...
            if (
                data.times &&
                Object.keys(data.times).length > 0
            ) {

                prayerTimes = data.times;

                if (data.hijri && islamicDate) {

                    islamicDate.innerHTML = data.hijri;
...
function detectLocation() {
    ...
    if (lat && lon) {
        ...
        const lastGps =
            localStorage.getItem("lastGpsUpdate");
    ...
    function handlePosition(lat, lon, accuracy) {
        ...
        if (acc > 3000) {
            ...
            if (!isRetry) {
                ...
                console.log("⚠️ Weak accuracy — retrying for real GPS...");
    ...
    navigator.geolocation.getCurrentPosition(
        ...
        error => {
            ...
            if (error.code === 1) {
                ...
                message = "⚠️ Location permission denied — settings me allow karein";

            } else if (error.code === 3) {

                message = "⚠️ Location timeout — GPS on karke dobara try karein";

            } else {

                message = "⚠️ Location nahi mili — saved location use kar rahe hain";
...
document.addEventListener(
    ...
    async () => {
        ...
        if (refreshBtnEl) {
            ...
            console.warn("⚠️ Location refresh button not found in DOM");
        ...
        const refreshAyahBtn =
            document.getElementById("refreshAyahBtn");

        if (refreshAyahBtn) {

            refreshAyahBtn.addEventListener("click", fetchDailyAyah);
        ...
        const refreshHadithBtn =
            document.getElementById("refreshHadithBtn");

        if (refreshHadithBtn) {

            refreshHadithBtn.addEventListener("click", refreshHadith);
...
function updateClock() {
    ...
    const ampmText =
        document.querySelector(".ampm-text");
...
async function getPrayerTimes(latitude, longitude, isBackgroundUpdate) {
    ...
    try {
        ...
        try {
            ...
            const cleanName =
                normalizeLocationName(detectedName);
        ...
        if (result.data.timings && result.data.timings.Maghrib) {
            ...
            const parts =
                result.data.timings.Maghrib.split(":");
...
function fetchDailyHadith() {
    ...
    localStorage.setItem(
        "dailyHadith",
        JSON.stringify({ date: today, index: randomIndex })
    );
